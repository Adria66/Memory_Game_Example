	...
	document.querySelectorAll('.card').forEach((card) => {
		card.addEventListener('click', () => {
			...
			if (memoryGame.pickedCards.length === 2) {
				const first = memoryGame.pickedCards[0].getAttribute('data-card-name');
				const second = memoryGame.pickedCards[1].getAttribute('data-card-name');
			...
			if (memoryGame.isFinished()) {
				document.body.innerHTML = '<h1>GAME IS OVER :)</h1>';
			}
		});
	});
});
